...
Voicing strategy is style-selectable and symbolically named
(\emph{Orpheus Black Moon}, \emph{Ares Blood Sun},
\emph{Nostradamus Dream Sigil}, \emph{Rasputin Mad Oracle}, \emph{Loki Mad Clock}).
Each style modifies chord density and extension behavior while preserving
the underlying functional decision (T/S/D), and each style name's gematria value
is used as a numeric driver for functional bias and extension selection in the model.

Seen musicologically, this is a layer of stylistic syntax over shared harmonic function:
\begin{itemize}
    \item \textbf{Orpheus Black Moon (188)}: compact consonant stacks with ``shadow'' extensions;
    legend-coded for memory/return gestures.
    \item \textbf{Ares Blood Sun (145)}: power-interval emphasis and root-fifth force;
    god-coded for high-drive attack.
    \item \textbf{Nostradamus Dream Sigil (242)}: dense tertian extension behavior with prophetic color tones;
    mystic-coded for harmonic overdetermination.
    \item \textbf{Rasputin Mad Oracle (190)}: dominant-heavy color and altered-tone inflection;
    madman-coded for unstable resolution pull.
    \item \textbf{Loki Mad Clock (109)}: functional harmony suspended into rotated drum-note mapping;
    trickster-coded for temporal rupture.
\end{itemize}
